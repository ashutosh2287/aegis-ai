...
export class ExerciseService {
  ...
  async getExerciseById(id: string): Promise<ExerciseWithRelations | null> {
    ...
    if (exerciseError) {
      if (exerciseError.code === 'PGRST116') { // No rows returned
        return null;
      }
      throw new InternalServerErrorException(`Failed to fetch exercise: ${exerciseError.message}`);
    }
  ...
  async createExercise(
    ...
    if (missingMuscles.length > 0) {
      throw new BadRequestException(`Muscle groups not found: ${missingMuscles.join(', ')}`);
    }

    const foundEquipmentNames = equipmentResult.data?.map((e: any) => e.name) || [];
    const missingEquipment = equipmentNames.filter(name => !foundEquipmentNames.includes(name));
    if (missingEquipment.length > 0) {
      throw new BadRequestException(`Equipment not found: ${missingEquipment.join(', ')}`);
    }
    ...
    if (insertError) {
      throw new InternalServerErrorException(`Failed to create exercise: ${insertError.message}`);
    }

    if (!insertedExercise) {
      throw new InternalServerErrorException('Exercise created but no data returned');
    }
    ...
    const exerciseEquipmentInserts = equipmentIds.map((equipmentId: string) => ({
      exercise_id: insertedExercise.id,
    ...
    if (exerciseMusclesInserts.length > 0) {
      ...
      if (muscleError) {
        ...
        throw new InternalServerErrorException(`Failed to create exercise-muscle relationships: ${muscleError.message}`);
      }
    }
    ...
    if (exerciseEquipmentInserts.length > 0) {
      ...
      if (equipmentError) {
        ...
        await this.supabaseService.getClient()
          ...
          .delete()
          .eq('id', insertedExercise.id);
        throw new InternalServerErrorException(`Failed to create exercise-equipment relationships: ${equipmentError.message}`);
      }
    }
  ...
  async updateExercise(
    ...
    if (existingExercise.isCustom && existingExercise.createdBy !== userId) {
      throw new BadRequestException('You are not authorized to update this exercise');
    }
    ...
    if (updateError) {
      throw new InternalServerErrorException(`Failed to update exercise: ${updateError.message}`);
    }

    if (!updatedExerciseData) {
      throw new InternalServerErrorException('Exercise updated but no data returned');
    }
    ...
    if (updateMuscleGroups !== undefined || updateEquipmentNeeded !== undefined) {
      ...
      if (updateMuscleGroups !== undefined) {
        ...
        if (musclesResult.error) {
          throw new InternalServerErrorException(`Failed to lookup muscles: ${musclesResult.error.message}`);
        }
      ...
      if (updateEquipmentNeeded !== undefined) {
        ...
        if (equipmentResult.error) {
          throw new InternalServerErrorException(`Failed to lookup equipment: ${equipmentResult.error.message}`);
        }
  ...
  async deleteExercise(id: string, userId: string): Promise<void> {
    ...
    // Check if the exercise is custom and if the user is the creator
    if (existingExercise.isCustom && existingExercise.createdBy !== userId) {
      throw new BadRequestException('You are not authorized to delete this exercise');
    }
    ...
    if (deleteError) {
      throw new InternalServerErrorException(`Failed to delete exercise: ${deleteError.message}`);
    }
  }
}