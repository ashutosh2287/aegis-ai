...
export class AuthService {
  ...
  async signup(
    ...
    const { data: authData, error: authError } =
      await this.tokenService.supabaseService.getClient().auth.signUp({
        email,
        password,
        options: {
          data: {
  ...
  async login(
    ...
    const { data: authData, error: authError } =
      await this.tokenService.supabaseService
        .getClient()
  ...
  async refreshToken(
    ...
    try {
      payload = await this.tokenService.jwtService.verifyAsync(refreshToken, {
        secret:
          this.tokenService.configService.get<string>("JWT_REFRESH_SECRET"),
      });
    } catch (error) {
    ...
    const isValid = await this.tokenService.isValidRefreshToken(
      userId,
      refreshToken,
    );
    if (!isValid) {
  ...
  async handleGoogleCallback(code: string) {
    ...
    let profile = await this.profileService
      .getProfile(user.id)
      .catch(() => null);
