...
export class WorkoutSetsService {
  ...
  async createSet(
    ...
    if (workoutError) {
      if (workoutError.code === "PGRST116") {
        // No rows returned
        throw new NotFoundException(
          `Workout with ID ${workoutExerciseData.workout_id} not found`,
        );
      }
      throw new InternalServerErrorException(
    ...
    if (workoutSetError) {
      throw new InternalServerErrorException(
        `Failed to create set: ${workoutSetError.message}`,
      );
    }

    if (!workoutSetDataResult) {
      throw new InternalServerErrorException(
        "Set created but no data returned",
      );
    }
  ...
  async updateSet(
    ...
    if (setError) {
      ...
      throw new InternalServerErrorException(
        `Failed to fetch set: ${setError.message}`,
      );
    }
    ...
    const { data: updatedSetData, error: updateError } =
      await this.supabaseService
        .getClient()
        .from("workout_sets")
        .update(updateData)
    ...
    if (updateError) {
      throw new InternalServerErrorException(
        `Failed to update set: ${updateError.message}`,
      );
    }

    if (!updatedSetData) {
      throw new InternalServerErrorException(
        "Set updated but no data returned",
      );
    }
  ...
  async deleteSet(
    ...
    if (deleteError) {
      throw new InternalServerErrorException(
        `Failed to delete set: ${deleteError.message}`,
      );
    }
  }
  ...
  async reorderSets(
    ...
    const invalidIds = dto.items
      .map((item) => item.id)
      .filter((id) => !existingSetIds.includes(id));
    if (invalidIds.length > 0) {
      throw new NotFoundException(
        `One or more set IDs not found: ${invalidIds.join(", ")}`,
      );
    }
    ...
    if (dto.items.length !== existingSetIds.length) {
      throw new BadRequestException(
        "The number of set IDs provided does not match the number of sets in the workout exercise",
      );
    }
    ...
    for (const item of dto.items) {
      ...
      if (updateError) {
        throw new InternalServerErrorException(
          `Failed to reorder set: ${updateError.message}`,
        );
      }
    }
  }
